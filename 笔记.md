...
![image-20230424121435159](笔记.assets/image-20230424121435159.png)
...
![image-20230424133432191](笔记.assets/image-20230424133432191.png)

发现是可以的，我们再来打印一下（或者通过vue开发者工具查看）

```js
console.log(store.state.cart,'cart');
```

![image-20230424133540161](笔记.assets/image-20230424133540161.png)
...
![image-20230424153849535](笔记.assets/image-20230424153849535.png)
...
```diff
    <div class="item" v-for="goods in $store.getters['cart/validList']" :key="item.skuId">
+          <RouterLink :to="`/product/${item.id}`">
            <img :src="item.picture" alt="">
```
...
使用 `src/views/cart/index.vue`（注册省略）
...
![image-20230425105509511](笔记.assets/image-20230425105509511.png)
...
![image-20230425111443344](笔记.assets/image-20230425111443344.png)
...
- 2.完成展开收起操作 ,展开的时候根据skuId得到商品信息（specs,skus）渲染商品规格`src/views/cart/components/cart-sku.vue`
...
![image-20230425133320739](笔记.assets/image-20230425133320739.png)
...
- 编写合并购物车的API接口函数 `src/api/cart.js`（前面已经给出）
...
为了方便，我们把用到的接口函数全部导入进来 api/cart.js
...
- 编写获取商品列表的API接口函数 `src/api/cart.js`（前面已给）
...
- 编写加入购物车的API接口函数 `src/api/cart.js`（前面已给）
...
```js
 insertCart(ctx,goods) {
      console.log(ctx);
      //console.log(goods);  // {skuId: '300456106', attrsText: '颜色：粉黑色(LXJ017) 尺码：26', stock: 5100, id: '4023638', name: '抓绒保暖，毛毛虫儿童运动鞋26-30', …}
      return new Promise((resolve,reject)=> {
         if(ctx.rootState.user.profile.token) {
            console.log('token');
...
- 编写删除购物车商品的API接口函数 `src/api/cart.js`（前面已给）
...
```js
   // 批量删除
    const batchDeleteCart = (isClear) => {
      ...
      // 清空失效商品：目前传入[]数据，会清空说有的商品包括有效商品（后台BUG）
```

## 二十一、登录后-选中状态&修改数量

- 编写修改购物车商品的API接口函数 `src/api/cart.js`（前面已给）
